...
void msp_gpio_init(uint8_t port)
{
    ysf_assert(IS_PORT_NUM_INVAILD(port));
...
void msp_gpio_fini(uint8_t port)
{
    ysf_assert(IS_PORT_NUM_INVAILD(port));
...
void msp_gpio_config(uint8_t port, uint8_t pin, uint8_t mode)
{
...
void map_gpio_init(struct map_gpio_t *gpio)
{
...
void map_gpio_fini(struct map_gpio_t *gpio)
{
    ysf_assert(IS_PTR_NULL(gpio));
    ysf_assert(IS_PORT_NUM_INVAILD(gpio->port));
...
void map_gpio_config(struct map_gpio_t *gpio, uint8_t mode)
{
    ysf_assert(IS_PTR_NULL(gpio));
    ysf_assert(IS_PORT_NUM_INVAILD(gpio->port));
